...
module Google
  module GCOMPUTE
    ...
    class TargetHttpProxy < Chef::Resource
      resource_name :gcompute_target_http_proxy
      ...
      action :create do
        fetch = fetch_resource(@new_resource, self_link(@new_resource), 'compute#targetHttpProxy')
        if fetch.nil?
          converge_by "Creating gcompute_target_http_proxy[#{new_resource.name}]" do
      ...
      action :delete do
        fetch = fetch_resource(@new_resource, self_link(@new_resource), 'compute#targetHttpProxy')
        unless fetch.nil?
          converge_by "Deleting gcompute_target_http_proxy[#{new_resource.name}]" do
      ...
      'application/json',
      {
        ...
        def wait_for_completion(status, op_result, resource)
          ...
          while status != 'DONE'
            ...
            sleep 1.0
            raise "Invalid result '#{status}' on gcompute_target_http_proxy." \
