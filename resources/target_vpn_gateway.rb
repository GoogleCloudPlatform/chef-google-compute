...
module Google
  module GCOMPUTE
    ...
    class TargetVpnGateway < Chef::Resource
      resource_name :gcompute_target_vpn_gateway
      ...
      action :create do
        fetch = fetch_resource(@new_resource, self_link(@new_resource), 'compute#targetVpnGateway')
        if fetch.nil?
          converge_by "Creating gcompute_target_vpn_gateway[#{new_resource.name}]" do
      ...
      action :delete do
        fetch = fetch_resource(@new_resource, self_link(@new_resource), 'compute#targetVpnGateway')
        unless fetch.nil?
          converge_by "Deleting gcompute_target_vpn_gateway[#{new_resource.name}]" do
      ...
      action_class do
        ...
        def update
          converge_if_changed do |_vars|
            ...
            puts # making a newline until we find a better way TODO: find!
            compute_changes.each { |log| puts "    - #{log.strip}\n" }
            message = 'TargetVpnGateway cannot be edited'
