...
module Google
  module GCOMPUTE
    ...
    class GlobalForwardingRule < Chef::Resource
      resource_name :gcompute_global_forwarding_rule
      ...
      action :create do
        ...
        if fetch.nil?
          converge_by ['Creating gcompute_global_forwarding_rule',
          ...
          @current_resource.region =
            ::Google::Compute::Property::RegionNameRef.api_parse(
              fetch['region']
            )
          @current_resource.target =
            ::Google::Compute::Property::String.api_parse(fetch['target'])
      ...
      action :delete do
        fetch = fetch_resource(@new_resource, self_link(@new_resource),
                               'compute#forwardingRule')
        unless fetch.nil?
          converge_by ['Deleting gcompute_global_forwarding_rule',
                       "[#{new_resource.name}]"].join do
      ...
      action_class do
        ...
        def wait_for_completion(status, op_result, resource)
          ...
          while status != 'DONE'
            ...
            sleep 1.0
            raise "Invalid result '#{status}' on gcompute_global_forwarding_rule." \
