...
module Google
  module GCOMPUTE
    ...
    class HttpHealthCheck < Chef::Resource
      resource_name :gcompute_http_health_check
      ...
      action :create do
        fetch = fetch_resource(@new_resource, self_link(@new_resource), 'compute#httpHealthCheck')
        if fetch.nil?
          converge_by "Creating gcompute_http_health_check[#{new_resource.name}]" do
          ...
          @current_resource.host = ::Google::Compute::Property::String.api_parse(fetch['host'])
          @current_resource.id = ::Google::Compute::Property::Integer.api_parse(fetch['id'])
          @current_resource.port = ::Google::Compute::Property::Integer.api_parse(fetch['port'])
          @current_resource.request_path =
            ::Google::Compute::Property::String.api_parse(fetch['requestPath'])
      ...
      action :delete do
        fetch = fetch_resource(@new_resource, self_link(@new_resource), 'compute#httpHealthCheck')
        unless fetch.nil?
          converge_by "Deleting gcompute_http_health_check[#{new_resource.name}]" do
      ...
      action_class do
        ...
        def wait_for_completion(status, op_result, resource)
          ...
          while status != 'DONE'
            ...
            sleep 1.0
            raise "Invalid result '#{status}' on gcompute_http_health_check." \
